...
function VertexList{Model}(site_count::Integer) where Model
    return VertexList(zeros(Int,0,0,0), zeros(Int, 2, site_count), zeros(Int, 2, site_count))
end

function make_vertex_list!(vl::VertexList{Model}, operators::AbstractVector{<:OperCode}, data::VertexData) where Model
    ...
    for (p, op) in enumerate(operators)
        ...
        b = get_bond_data(data, get_bond(op))
        for s in 1:leg_count(Model)÷2
            (s1, p1) = vl.v_last[:,b[s]]
            if p1 != -1
            ...
            vl.v_last[:, b[s]] .= (leg_count(Model)÷2 + s, p)
        end
    end

    for i in 1:size(vl.v_first, 2) 
        if vl.v_first[1,i] != -1
